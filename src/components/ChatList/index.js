...
export const ChatList = ({ data, setChatUser, isChatTab }) => {
    ...
    const removeuser = async (empId) => {
        try {
            const userResponse = await axios.post(`http://44.203.55.138:2222/api/User/DeleteUser?empId=${empId}`);
    ...
    const getChat = async (data) => {
        try {
            const res = await axios.get(`http://44.203.55.138:2222/api/User/GetChat?empName={${data.receiver}}`)
